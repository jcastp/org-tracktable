...
- modified the word count function (=org-tracktable-word-count=) that now ignores:
  - org comment lines (=org-in-commented-line=)
  - org drawers (=org-in-drawer-p=)
  - org headings (=org-in-heading-p=)
  - org properties (=org-at-property-p=)
  - org blocks (=org-in-block-p=)
  - ignore the text in headings with particular tags, that can be defined by the user (=org-under-heading-with-tag-p=)
